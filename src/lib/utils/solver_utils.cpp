...
void set_var_suffix(const std::string &suffix) { g_var_suffix = suffix; }

std::string get_var_suffix() { return g_var_suffix; }
...
expr parseFormula(solver_context &sol_ctx, std::string str,
                  const std::vector<std::string> &names,
                  const std::vector<expr> &declarations) {
  ...
  // std::cout << "Cmd is " << cmd << "\n";
  // Z3_ast_vector es_ast = Z3_parse_smtlib2_string(sol_ctx, cmd.c_str(), 0,
  // NULL, NULL, s, symbols, decls); delete[] symbols; delete[] decls;
  ...
  // std::cout << "Cmd is " << cmd << "\n";
  try {
    Z3_ast_vector es_ast = Z3_parse_smtlib2_string(
        sol_ctx, cmd.c_str(), 0, NULL, NULL, s, symbols, decls);
    delete[] symbols;
    delete[] decls;
    if (es_ast == NULL) {
      llvm_bmc_error("parsing", "failed to parse input: " << str);
...
void print_expr(std::ostream &o, expr e) { o << e << "\n"; }

void print_sort(std::ostream &o, sort e) { o << e << "\n"; }

void dump(expr e) { print_expr(std::cerr, e); }

void dump(exprs es) { print_exprs(std::cerr, es); }

void dump(expr_set es) { print_expr_set(std::cerr, es); }

void dump(expr_vector es) { print_expr_vec(std::cerr, es); }

void dump(sort e) { print_sort(std::cerr, e); }
...
std::string display(expr e, const std::map<std::string, std::string> &rename) {
  if (e.is_numeral()) {
    int64_t num, den;
...
bool isRel(expr e, std::string s, bool th) {
  return isGE(e, s, th) || isGT(e, s, th) || isLE(e, s, th) || isLT(e, s, th) ||
         isEQ(e, s, th) || isNot(e, s, th);
}
...
bool is_false(expr f) { return eq(f, f.ctx().bool_val(false)); }

bool is_true(expr f) { return eq(f, f.ctx().bool_val(true)); }

expr get_expr_const(solver_context &c, int num) { return c.int_val(num); }
...
expr LogShR(expr const &a, expr const &b) {
  if (a.is_bv()) {
    ...
    llvm_bmc_error(
        "LogShR",
        "Shift operations are only supported for bitvectors and integers!");
...
expr switch_int_sort(expr &b, sort &s) {
  ...
  if (bs.is_bool() && s.is_int()) {
  ...
  } else if (bs.is_int() && s.is_int()) {
    // Both are int, already compatible
    return b;
    ...
    if (is_const(b)) {
      int64_t v = 0;
      if (Z3_get_numeral_int64(b.ctx(), b, &v)) {
        return b.ctx().int_val((int)v);
      }
    }
    ...
    return to_expr(b.ctx(), Z3_mk_real2int(b.ctx(), b));
  }
  dump(b);
  dump(s);
...
//     assert(t.is_bv());
//     Z3_ast r = Z3_mk_fpa_to_fp_signed(t.ctx(), t.ctx().fpa_rounding_mode(),
//     t, s); return expr(t.ctx(), r);
//      }
...
expr substitute(expr e, std::vector<expr> &from, std::vector<expr> &to) {
...
void get_variables(expr &e, expr_set &vars) {
  ...
  case Z3_QUANTIFIER_AST: {
    ...
    // vars.insert(expr(expr.ctx(), Z3_mk_const(expr.ctx(), sym,
    // Z3_mk_int_sort(expr.ctx()))));
    expr body = e.body();
...
void inplace_substitute(expr &v, expr_vector &outgoing, expr_vector &incoming) {
...
void inplace_substitute(exprs &vec, expr &outgoing, expr &incoming) {
    ...
    while (it != list.end()) {
      ...
      if (isEQ(e)) {
        if (e.arg(0).is_array() && isStore(e.arg(1)) && !isStore(e.arg(0))) {
          ...
          list.erase(it);
          incr_flag = false;
        } else if (e.arg(1).is_array() && isStore(e.arg(0)) &&
                   !isStore(e.arg(1))) {
...
bool isBool(const std::string &s) { return s == "false" || s == "true"; }
...
std::string balancehelper(std::string line) {
  ...
  // std::cout<<"Prefix: "<<prefix<< " Middle : " << middle << "Suffix : " <<
  // suffix <<"\n"; return "";
  return prefix + balanceOrAnd(middle) + suffix;
}

std::string dumpmodel(std::string dump_path, std::string name,
                      std::string solver) {
...
std::vector<std::string> tokenize_boolector(std::string s,
                                            std::string del = " ") {
  // std::cout<<"in token\n"<<s<<"\n";
  std::vector<std::string> exp;
  int end = s.find(del);
...
std::vector<std::vector<std::string>> getExpressions(std::string filePath,
...
void Z3CompClass::Z3compatible(std::string path, std::string solvertype) {
  ...
  *temp = s.get_model();
  ...
  std::cerr << "dumping Z3compatible-" << solvertype
            << " model in:" << path + name << "\n";
  dump_model(*m, path, name);
}

check_result Z3CompClass::check_cvc5(solver &s, std::string outDirPath,
                                     bool dump_model) {
  dump(outDirPath, "test.smt2", s);

  std::string cmd = "./cvc5 --lang=smt2 --dump-models " + outDirPath +
                    "test.smt2 >> tempFile";
  std::string sDirectory = "build/cvc5/build/bin";
  cleancvc5(outDirPath + "/test.smt2",
            outDirPath + "/temp.smt2"); // Clean smt2 file
...
check_result Z3CompClass::check_boolector(solver &s, std::string outDirPath,
                                          bool dump_model) {
  dump(outDirPath, "test.smt2", s);

  std::string cmd =
      "./boolector --smt2 -m " + outDirPath + "test.smt2 >> tempFile";
  std::string sDirectory = "build/boolector/build/bin";
  cleanboolector(outDirPath + "/test.smt2",
                 outDirPath + "/temp.smt2"); // Clean smt2 file
  if (chdir(sDirectory.c_str()) == -1)
