...
void verify_prop_pass::init_parse(llvm::Module &m, options& o)
{
	...
	auto CallSeqpair = i->second;	  
        std::string FnName1 = CallSeqpair.first;
	std::string FnName2 = CallSeqpair.second;
	//std::cout << "Fn1 is " << FnName1 << " Fn2 is " << FnName2 << " Thread is " << ThName << " Entry fn. is " << EntryFnName << "\n";
...
void verify_prop_pass::insert_monitor(llvm::Module &m, std::string Fn1, std::string Fn2)
{
    ...
    invokedFn_Val->print( llvm::outs() );     std::cout << "\n";
...
{    
	...
	llvm::Instruction *icmp1 = new llvm::ICmpInst(llvm::ICmpInst::ICMP_EQ, invokedFn_Val, i32_val1, cmp_res_name); 
	...
	//m.print( llvm::outs(), AAW, false, false );     std::cout << "\n";
...
//std::cout << "Called fn is " << FuncName << "\n";
					if (fname1 == FuncName) {
						...
						llvm::Instruction *icmp = new llvm::ICmpInst(I,llvm::ICmpInst::ICMP_ULT, invokedFn_Val, i32_val2); // cmp_res_name);
						...
						llvm::SelectInst *select = llvm::SelectInst::Create (icmp, const_val1, const_val2, monitor_var_name, I);
						select->print( llvm::outs() );     std::cout << "\n";
						//break;	
					}

					if (fname2 == FuncName) {
						llvm::Instruction *icmp = new llvm::ICmpInst(I,llvm::ICmpInst::ICMP_EQ, invokedFn_Val, i32_val1); // cmp_res_name);
