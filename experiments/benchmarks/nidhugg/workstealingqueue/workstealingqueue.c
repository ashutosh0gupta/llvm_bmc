...
typedef struct WorkStealQueue {
  ...
  atomic_int MaxSize;
  atomic_int InitialSize; // must be a power of 2

  atomic_int head;  // only updated by Take
  atomic_int tail;  // only updated by Push and Pop

  Obj*  elems[STATICSIZE];         // the array of tasks
  atomic_int mask;           // the mask for taking modulus

} WorkStealQueue;

WorkStealQueue q;

int my_atomic_exchange(atomic_int *obj, int v) {
  __my_atomic_begin();
  int t = atomic_load_explicit(obj, memory_order_relaxed);
  atomic_store_explicit(obj, v, memory_order_relaxed);
  __my_atomic_end();
  return t;
}

_Bool my_atomic_compare_exchange_strong(atomic_int * obj, int* expected, int desired) {
  int ret = 0;
  __my_atomic_begin();
...
_Bool Steal(Obj **result) {
  ...
  if (h < readV(&q.tail)) {
    ...
    int temp = h & atomic_load_explicit(&q.mask, memory_order_relaxed);
    *result = q.elems[temp];
    found = 1;
  } else {
...
_Bool SyncPop(Obj **result) {
  ...
  if (readV(&q.head) <= t) {
    // == (head <= tail)
    //
    int temp = t & atomic_load_explicit(&q.mask, memory_order_relaxed);
    *result = q.elems[temp];
    found = 1;
  } else {
...
void SyncPush(Obj* elem) {
  ...
  if (count >= atomic_load_explicit(&q.mask, memory_order_relaxed)) {
    // == (count >= size-1)
    //
    int newsize = (atomic_load_explicit(&q.mask, memory_order_relaxed) == 0 ?
    ...
    int i;
    for (i = 0; i < count; i++) {
      int temp = (h + i) & atomic_load_explicit(&q.mask, memory_order_relaxed);
