...
static atomic_char buffer[BUFFER_MAX];   /* BUFFER */				

static atomic_int first;        /* Variable to point to the input buffer   */
static atomic_int next;         /* Variable to point to the output pointer */
static atomic_int buffer_size;  /* Max amount of elements in the buffer    */

atomic_int send, receive;
atomic_int value;
