...
struct mpmc_boundq_1_alt
{
	...
	unsigned long    m_rdwr;		// A workaround
	// "read" and "written" count the number completed
	unsigned long    m_read;		// A workaround
	unsigned long 	m_written;		// A workaround
};
...
t_element *read_fetch(mpmc_boundq_1_alt *q)
{
	...
	for (int i = 0u; i <= LB; i++) {
		if (i == LB)
			return NULL;
		if ((atomic_load_explicit(&q->m_written, mo_acquire) & 0xFFFF) == wr)
			break;
	}
...
t_element *write_prepare(mpmc_boundq_1_alt *q)
{
	...
	for (int i = 0u; i <= LB; i++) {
		if (i == LB)
			return NULL;
		if ((atomic_load_explicit(&q->m_read, mo_acquire) & 0xFFFF) == rd )
			break;
	}
