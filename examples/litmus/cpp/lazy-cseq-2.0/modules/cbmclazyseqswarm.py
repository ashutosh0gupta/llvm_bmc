...
Changelog:
    2017.02.09  initial version (from lazyseqnewscheduleswarm.py 2016.11.30)
...
class cbmclazyseqswarm(core.module.Translator):
    ...
    def loadfromstring(self, string, env):
        ...
        self.__stmtCount = -1         # statement counter to
        ...
        header = core.utils.printFile('modules/cbmclazyseqA.c')
        ...
        header = core.utils.printFile('modules/cbmclazyseqB.c').replace('<insert-threadsizes-here>',lines)
        ...
        if n.block_items:
            for stmt in n.block_items:
                ...
                if type(stmt) == pycparser.c_ast.FuncCall and stmt.name.name == core.common.changeID['pthread_exit']: ##if type(stmt) == pycparser.c_ast.FuncCall and self._parenthesize_unless_simple(stmt.name) == core.common.changeID['pthread_exit']:
                    ...
                    stamp = '__CSEQ_rawline("%s%s_%s: ");' % (self.__startChar, self.__currentThread, str(self.__realStmtCount))
                    stamp += '__CSEQ_atomic_end();\n'
                ...
                # Case 2: labels
                    ...
                    if not self.__atomic and self.__realStmtCount == -1:   # first statement in a thread
                        ...
                        stamp = '__CSEQ_rawline("IF(%s,%s,%s%s_%s)");' % (threadIndex,str(self.__realStmtCount), self.__startChar, self.__currentThread, str(self.__realStmtCount+1))
                        stamp += '__CSEQ_atomic_begin();\n'
                        code = self.visit(stmt)
                        newStmt = stamp + code + ';\n'
                    elif (
                        ...
                        if self._is_left_bound(self.__stmtCount, self.__currInterval):
                            ...
                            self.__maxInCompound = self.__realStmtCount
                            threadIndex = self.Parser.threadIndex[self.__currentThread] if self.__currentThread in self.Parser.threadIndex else 0
                            # stamp = '__CSEQ_rawline("%s%s_%s: IF(%s,%s,%s%s_%s)");' % (self.__startChar, self.__currentThread, str(self.__realStmtCount),threadIndex,str(self.__realStmtCount), self.__startChar, self.__currentThread, str(self.__realStmtCount+1))
                            stamp = '__CSEQ_rawline("IF(%s,%s,%s%s_%s)");' % (threadIndex,str(self.__realStmtCount), self.__startChar, self.__currentThread, str(self.__realStmtCount+1))
                            stamp += '__CSEQ_atomic_end();\n'
    ...
    def visit_FuncDef(self, n):
        ...
        # swarm, set interval for each thread
        if n.decl.name == 'main':
    ...
    def visit_FuncCall(self, n):
        ...
        #                          ^^^
        #
        elif fref == core.common.changeID['pthread_create']: # TODO re-write AST-based (see other modules)
