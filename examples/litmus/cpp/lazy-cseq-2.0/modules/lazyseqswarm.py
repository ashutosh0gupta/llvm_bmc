...
"""
VERSION = 'lazyseqswarm-0.1-2016.09.14'   # CSeq-1.3 Release for journal
# VERSION = 'lazyseq-0.1-2015.10.19'   # CSeq-1.3 Release for journal
...
class lazyseqswarm(core.module.Translator):
    ...
    def visit_Compound(self, n):
        ...
        if n.block_items:
            for stmt in n.block_items:
                ...
                if isinstance(stmt, pycparser.c_ast.FuncCall) and stmt.name.name == core.common.changeID['pthread_exit']: ##if type(stmt) == pycparser.c_ast.FuncCall and self._parenthesize_unless_simple(stmt.name) == core.common.changeID['pthread_exit']:
                    self.__stmtCount += 1
                ...
                # Case 2: labels
                    ...
                    if not self.__atomic and self.__realStmtCount == -1:   # first statement in a thread
                        ...
                        (isinstance(stmt, pycparser.c_ast.FuncCall) and stmt.name.name == '__CSEQ_atomic_begin') or
                        (not self.__atomic and
                            (globalAccess or
                            (isinstance(stmt, pycparser.c_ast.FuncCall) and stmt.name.name == core.common.changeID['pthread_create']) or
                            (isinstance(stmt, pycparser.c_ast.FuncCall) and stmt.name.name == core.common.changeID['pthread_join']) or
                            (isinstance(stmt, pycparser.c_ast.FuncCall) and stmt.name.name.startswith('__CSEQ_atomic') and not stmt.name.name == '__CSEQ_atomic_end') or
                            (isinstance(stmt, pycparser.c_ast.FuncCall) and stmt.name.name.startswith('__CSEQ_assume')) or
                            (isinstance(stmt, pycparser.c_ast.FuncCall) and stmt.name.name == '__cs_cond_wait_2')
                            )
                        )
                        )):
                        self.__stmtCount += 1
