...
"""
VERSION = 'lazyseqnewscheduleinline-0.1-2016.12.05'
...
class lazyseqnewscheduleinline(core.module.Translator):
    ...
    # For inlining thread function
    __threadCopy = {}     # Copy of thread to generate for different round
    __threadVars = {}     # Local variable of each thread
    __inThreadFunc = False
    ...
    def loadfromstring(self, string, env):
        ...
        if backend == 'klee':    # specific main for klee
            ...
            self.output += self.__createMainRoundRobinFramaC(rounds)
        else:
            # Add the new main().
        ...
        if n.block_items:
            for stmt in n.block_items:
                ...
                # Case 2: labels
                    ...
                    newStmt = self._make_indent()+ '<round>' + stmt.name + ': ' + guard + newStmt+ '\n'
    ...
    def visit_FuncDef(self, n):
        ...
        decl = self.visit(n.decl)
        self.indent_level = 0
        self.__threadVars[self.__currentThread] = []  # Initialize variables of thread
        ...
        if self.__currentThread == 'main':
            ...
            if main_args.find('void') != -1 or main_args == '':
                ...
                # args += main_args[1] + ';'
                self.__threadVars['main'].append(main_args[0])
                self.__threadVars['main'].append(main_args[1])
    ...
    def __createMainRoundRobinFramaC(self, ROUNDS):
        '''  New main driver:
        '''
        main = ''
        ...
        temp = self.__threadCopy['main'].replace('<round>', 'R0_main_')
        main +="          %s\n" % (temp)
        main +="          __cs_pc[0] = __cs_pc_cs;\n"
        ...
        for t in self.__threadName:
            ...
            if i <= self.__threadbound:
                ...
                temp = self.__threadCopy[t].replace('<round>', 'R0_%s_' % t)
                main +="             %s\n" % (temp)
                main +="             __cs_pc[%s] = __cs_pc_cs;\n" % (i)
        ...
        for round in range(1, ROUNDS):
            ...
            main +="             %s\n" % self._generateConstraintsFramaC('__cs_pc_cs', self.__lines['main'])
            temp = self.__threadCopy['main'].replace('<round>', 'R%s_main_' % round)
            main +="             %s\n" % (temp)
            main +="             __cs_pc[0] = __cs_pc_cs;\n"
            main +="             %s\n" % self._generateConstraintsFramaC('__cs_pc[0]', self.__lines['main'])
            main +="          }\n\n"
            ...
            for t in self.__threadName:
                ...
                if i <= self.__threadbound:
                    ...
                    main +="             %s\n" % self._generateConstraintsFramaC('__cs_pc_cs', self.__lines[t])
                    temp = self.__threadCopy[t].replace('<round>', 'R%s_%s_' % (round, t))
                    main +="             %s\n" % (temp)
                    main +="             __cs_pc[%s] = __cs_pc_cs;\n" % (i)
                    main +="             %s\n" % self._generateConstraintsFramaC('__cs_pc[%s]' % i, self.__lines[t])
                    main +="         }\n\n"
        ...
        main +="             __cs_pc_cs = Frama_C_unsigned_int_interval(__cs_pc[0], %s);\n" % (self.__lines['main'])
        main +="             %s\n" % self._generateConstraintsFramaC('__cs_pc_cs', self.__lines['main'])
        temp = self.__threadCopy['main'].replace('<round>', 'R%s_main_' % ROUNDS)
        main +="             %s\n" % temp
        main +="           }\n"
