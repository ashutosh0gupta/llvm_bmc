...
SCRIPT = 'cbmc'
LOGFILENAME = curDir + '/../sc.cbmc.results.txt'
...
def runall():
    ...
    logfile.write('# cbmc --mm sc TEST.c\n')
    logfile.write('#\n')
    ...
    logfile.write('# \n')
    logfile.write('# The tests where executed using runall-CBMC-SC.py.\n')
    ...
    for tst in tests:
        n = n + 1
        try:
            cmd = SCRIPT + ' --mm sc' + ' ' + LITMUSDIR + '/' + tst['tstname'] + '.c'
            # print(cmd)
