/* Copyright (C) 2023 ARM-CBMC
* This benchmark is part of ARM-CBMC */

#include <assert.h>
#include <pthread.h>
#include <stdatomic.h>

// Memory barriers
void dmbld();
void dmbst();
void dmbsy();
void isb();
// ARM-CBMC specific functions to support exclusive accesses
void ldx(int *);
void ldax(int *);
void stx(int *, int);
void stlx(int *, int);

int vars[2]; 
int atom_0_X5_1; 
int atom_2_X4_3; 
int atom_1_X2_4; 
int atom_1_X4_1; 

void *t0(void *arg){
label_1:;
  atomic_store_explicit(&vars[0], 2, memory_order_release);
  int v1_W5 = atomic_load_explicit(&vars[0], memory_order_relaxed);
  atomic_store_explicit(&vars[0], 3, memory_order_relaxed);
  atomic_store_explicit(&vars[0], 4, memory_order_relaxed);
  int v12 = (v1_W5 == 1);
  atom_0_X5_1 = v12;
  return NULL;
}

void *t1(void *arg){
label_2:;
  int v2_W2 = atomic_load_explicit(&vars[0], memory_order_acquire);
  int v3_W4 = atomic_load_explicit(&vars[1], memory_order_acquire);
  int v14 = (v2_W2 == 4);
  atom_1_X2_4 = v14;
  int v15 = (v3_W4 == 1);
  atom_1_X4_1 = v15;
  return NULL;
}

void *t2(void *arg){
label_3:;
  atomic_store_explicit(&vars[1], 1, memory_order_relaxed);
  atomic_store_explicit(&vars[1], 2, memory_order_release);
  atomic_store_explicit(&vars[0], 1, memory_order_release);
  int v4_W4 = atomic_load_explicit(&vars[0], memory_order_relaxed);
  int v13 = (v4_W4 == 3);
  atom_2_X4_3 = v13;
  return NULL;
}

int main(int argc, char *argv[]){
  pthread_t thr0; 
  pthread_t thr1; 
  pthread_t thr2; 

  atomic_init(&vars[1], 0);
  atomic_init(&vars[0], 0);
  atom_0_X5_1 = 0; 
  atom_2_X4_3 = 0; 
  atom_1_X2_4 = 0; 
  atom_1_X4_1 = 0; 

  pthread_create(&thr0, NULL, t0, NULL);
  pthread_create(&thr1, NULL, t1, NULL);
  pthread_create(&thr2, NULL, t2, NULL);

  pthread_join(thr0, NULL);
  pthread_join(thr1, NULL);
  pthread_join(thr2, NULL);

  int v5 = atom_0_X5_1;
  int v6 = atom_2_X4_3;
  int v7 = atom_1_X2_4;
  int v8 = atom_1_X4_1;
  int v9_conj = v7 & v8;
  int v10_conj = v6 & v9_conj;
  int v11_conj = v5 & v10_conj;
  if (v11_conj == 1) assert(0);
  return 0;
}
