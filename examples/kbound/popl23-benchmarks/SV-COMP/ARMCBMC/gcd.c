...
void atomic_dec_a(void)
{
	pthread_mutex_lock(&lock);
  	if(atomic_load_explicit(&a, memory_order_relaxed) > atomic_load_explicit(&b, memory_order_relaxed))
	{
		int _a = atomic_load_explicit(&a, memory_order_relaxed);
		int _b = atomic_load_explicit(&b, memory_order_relaxed);
		atomic_store_explicit(&a, _a - _b, memory_order_relaxed);
	}
...
{
	pthread_mutex_lock(&lock);
  	if(atomic_load_explicit(&b, memory_order_relaxed) > atomic_load_explicit(&a, memory_order_relaxed))
	{
		int _a = atomic_load_explicit(&a, memory_order_relaxed);
		int _b = atomic_load_explicit(&b, memory_order_relaxed);
		atomic_store_explicit(&b, _b - _a, memory_order_relaxed);
	}
	pthread_mutex_unlock(&lock);
}
