...
define dso_local fastcc void @mul_add_mat_x_m_mat(i32 noundef %m_vec_limbs, ptr noundef nonnull %mat, ptr noundef nonnull %bs_mat, ptr noundef nonnull %acc, i32 noundef %mat_rows, i32 noundef %mat_cols, i32 noundef range(i32 8, 11) %bs_mat_cols) unnamed_addr #0 {
entry:
...
for.body6:                                        ; preds = %for.cond4.preheader, %m_vec_mul_add.exit
  ...
  %mul = mul nuw nsw i32 %c.03, %bs_mat_cols
  %add = add nuw nsw i32 %mul, %k.02
  %add.ptr.idx = mul nuw nsw i32 %add, 40
  %add.ptr = getelementptr inbounds nuw i8, ptr %bs_mat, i32 %add.ptr.idx
  %mul8 = mul nuw nsw i32 %r.04, 78
  %0 = getelementptr i8, ptr %mat, i32 %mul8
  %arrayidx = getelementptr i8, ptr %0, i32 %c.03
  %1 = load i8, ptr %arrayidx, align 1
  %mul10 = mul nuw nsw i32 %r.04, %bs_mat_cols
  %add11 = add nuw nsw i32 %mul10, %k.02
...
m_vec_mul_add.exit:                               ; preds = %for.body.i
  ...
  %exitcond.not = icmp eq i32 %add14, %bs_mat_cols
...
for.inc17:                                        ; preds = %for.inc15
  ...
  %exitcond2.not = icmp eq i32 %inc18, 10
