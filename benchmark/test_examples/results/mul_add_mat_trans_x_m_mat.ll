...
define dso_local fastcc void @mul_add_mat_trans_x_m_mat(i32 noundef %m_vec_limbs, ptr noundef nonnull %mat, ptr noundef nonnull %bs_mat, ptr noundef nonnull %acc, i32 noundef %mat_rows, i32 noundef %mat_cols, i32 noundef %bs_mat_cols) unnamed_addr #0 {
entry:
...
for.body6:                                        ; preds = %for.cond4.preheader, %m_vec_mul_add.exit
  ...
  %mul = shl nuw nsw i32 %c.03, 3
  %add = or disjoint i32 %mul, %k.02
  %add.ptr.idx = mul nuw nsw i32 %add, 40
  %add.ptr = getelementptr inbounds nuw i8, ptr %bs_mat, i32 %add.ptr.idx
  %mul8 = shl nuw nsw i32 %c.03, 3
  %0 = getelementptr i8, ptr %mat, i32 %mul8
  %arrayidx = getelementptr i8, ptr %0, i32 %r.04
  %1 = load i8, ptr %arrayidx, align 1
  %mul10 = shl nuw nsw i32 %r.04, 3
  %add11 = or disjoint i32 %mul10, %k.02
...
for.body.i:                                       ; preds = %for.body6, %for.body.i
  ...
  %arrayidx24.i = getelementptr inbounds nuw i64, ptr %add.ptr13, i32 %i.0.i1
...
m_vec_mul_add.exit:                               ; preds = %for.body.i
  %add14 = add nuw nsw i32 %k.02, 1
  %exitcond.not = icmp eq i32 %add14, 8
...
for.inc15:                                        ; preds = %m_vec_mul_add.exit
  ...
  %exitcond1.not = icmp eq i32 %inc, 78
  br i1 %exitcond1.not, label %for.inc17, label %for.cond4.preheader, !llvm.loop !8

for.inc17:                                        ; preds = %for.inc15
  %inc18 = add nuw nsw i32 %r.04, 1
  %exitcond2.not = icmp eq i32 %inc18, 8
